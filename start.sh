...
# start Openbravo POS
java -cp $CP -Djava.util.logging.config.file=$DIRNAME/logging.properties -Dswing.defaultlaf=javax.swing.plaf.metal.MetalLookAndFeel -Djava.library.path=$DIRNAME$LIBRARYPATH -Ddirname.path=$DIRNAME/ -Dlog4j.configuration=file:$DIRNAME/openbravopos-log4j.properties com.openbravo.pos.forms.StartPOS "$@"
